...
import { createBlog, updateBlog, deleteBlog } from "@/services/user"
...
export default function BlogEditor({ postId }: { postId?: string }) {
  ...
  useEffect(() => {
    if (postId) {
      ...
      const fetchPost = async () => {
        try {
          const response = await fetch(`${endpoints.blog.GET_BLOG}${postId}`)
  ...
  const handleSave = async (newStatus?: "draft" | "published") => {
    ...
    try {
      ...
      if (postId) {
        await updateBlog(postId, blogData)
      } else {
  ...
  const handleDelete = async () => {
    ...
    try {
      await deleteBlog(postId as string)
      router.push("/blog")
    } catch (error) {
